...
def first_trust_based_update(row, s, r):
	# a: True, b: True
	# df['First_trust_based_updated'] = df['Initial']
	df = pd.read_csv('truth_table.csv')

	for index, row in df[(df['a'] == True) & (df['b'] == True)].iterrows():
		add_sum = df[((df['a'] == False) | (df['b'] == False)) & ((df['c'] == row['c']) & (df['d'] == row['d']) & (df['e'] == row['e']) & (df['f'] == row['f']) & (df['g'] == row['g']) & (df['h'] == row['h']) & (df['i'] == row['i']) & (df['j'] == row['j']) & (df['k'] == row['k']))]['Initial'].sum()
		# print(add_sum)
...
def first_response_based_update():
	...
	for index, row in df[(df['c'] == False) & (df['a'] == False)].iterrows():
		add_sum = df[((df['a'] == True) | (df['c'] == True)) & ((df['b'] == row['b']) & (df['d'] == row['d']) & (df['e'] == row['e']) & (df['f'] == row['f']) & (df['g'] == row['g']) & (df['h'] == row['h']) & (df['i'] == row['i']) & (df['j'] == row['j']) & (df['k'] == row['k']))]['First_trust_based_updated'].sum()
		# print(add_sum)
	...
	for index, row in df[(df['b'] == False) | (df['a'] == False) | (df['c'] == True)| (df['d'] == False)].iterrows():
		add_sum = df[((df['a'] == True) & (df['c'] == False)) & ((df['b'] == True) & (df['d'] == True) & (df['e'] == row['e']) & (df['f'] == row['f']) & (df['g'] == row['g']) & (df['h'] == row['h']) & (df['i'] == row['i']) & (df['j'] == row['j']) & (df['k'] == row['k']))]['First_response_based_updated'].sum()
		# print(add_sum)
		df.at[index, 'First_response_based_updated'] = row['First_response_based_updated'] + s * add_sum

	df.loc[(df['a'] == True) & (df['c'] == False) & (df['b'] == True) & (df['d'] == True), 'First_response_based_updated'] *= (1 - s)
...
def round1_calculation(row):
	...
	human_ranking = [int(row['Ranking A_1']), int(row['Ranking A_2']), int(row['Ranking A_3']), int(row['Ranking A_4'])]
